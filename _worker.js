...
async function router(path, method, url, request, SB, KEY, env = {}) {
  ...
  // RIGS
  if (res === 'rigs') {
    if (method === 'GET' && !id) return ok(await sbGet(SB, KEY, 'rigs', { order: 'rig_name.asc' }));
  ...
  if (res === 'contracts') {
    if (method === 'GET') {
      const { data, error } = await sbGet(SB, KEY, 'contracts', { select: '*, rigs(rig_name), contract_assets(asset_id)', order: 'id.asc', limit: +(q.get('limit') || 200) });
      if (error) return err500(error);
      return ok((data || []).map(c => ({ ...c, rig: c.rigs?.rig_name, asset_count: (c.contract_assets || []).length, rigs: undefined, contract_assets: undefined })));
    }
  ...
  if (res === 'transfers') {
    if (method === 'POST' && id && act === 'approve') {
      ...
      if (role === 'ops') {
        patch = {
          ops_approved_by: approved_by, ops_approved_date: today, ops_action: decision, ops_comment: comment,
          status: decision === 'approve' ? 'Ops Approved' : decision === 'reject' ? 'Rejected' : 'On Hold'
        };
      } else if (role === 'mgr') {
        patch = {
          mgr_approved_by: approved_by, mgr_approved_date: today, mgr_action: decision, mgr_comment: comment,
